...
class ModelHistory(object):
    ...
    def get_coeffs_variation(self):
        ...
        for chain in num.arange(self.nchains):
            ...
            multivar_coeff_variation = num.sqrt((
                mstd.T * mcov * mstd / norm_mstd))
...
def load_problem_info_and_data(dirname, subset=None, nchains=None):
    ...
    models, misfits, bootstraps, sampler_contexts = \
        load_problem_data(xjoin(dirname, subset), problem, nchains=nchains)
...
def load_problem_data(dirname, problem, nmodels_skip=0, nchains=None):
    ...
    try:
        ...
        if op.exists(fn):
            ...
            nsampler_context = filesize // nmodels // 8

            with open(fn, 'r') as f:
                f.seek(nmodels_skip * nsampler_context * 8)
                sampler_contexts = num.fromfile(
                        f, dtype='<i8',
                        count=nmodels*nsampler_context)\
                    .astype(num.int)

                sampler_contexts = sampler_contexts\
                    .reshape((nmodels, nsampler_context))
